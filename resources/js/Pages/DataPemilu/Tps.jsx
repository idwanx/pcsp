...
export default function Tps(props) {
    ...
    const storeHandler = (e) => {
        e.preventDefault();
        post(route('datapemilu.storetps'), {
            preserveScroll: true,
            onSuccess: () => {
                closeFormAdd();
            },
    ...
    const updateHandler = (e) => {
        e.preventDefault();
        put(route('datapemilu.updatetps', data.id), {
            preserveScroll: true,
            onSuccess: () => {
                closeFormUpdate();
            },
    ...
    const destroyHandler = () => {
        router.delete(route('datapemilu.destroytps', state.id), { 
            preserveScroll: true,
            onSuccess: () => {
                closeDestroy();
            },
    ...
    return (
        <>
            ...
            <Modal isOpen={isFormAdd} closeable={false} onClose={closeFormAdd} size={`sm`}>
                <TpsForm isClose={closeFormAdd} data={data} setData={setData} errors={errors} processing={processing} submit={storeHandler} submitLabel={`Tambah`} title={`Tambah TPS`} />
            </Modal>
            <Modal isOpen={isFormUpdate} closeable={false} onClose={closeFormUpdate} size={`sm`}>
                <TpsForm isClose={closeFormUpdate} data={data} setData={setData} errors={errors} processing={processing} submit={updateHandler} submitLabel={`Update`} title={`Update TPS`} />
            </Modal>
                ...
                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        <p className="text-sm text-gray-500">
                            Apakah anda yakin <span className='text-red-500'>{state.nama_tpsuara}</span> akan dihapus?
                        </p>
                        </div>
                    </div>
                    </div>
                                    ...
                                    <table className="w-full table-fixed">
                                        <thead className="uppercase text-xs rounded-t-lg bg-gray-100">
                                            <tr className="text-center font-semibold text-gray-600 border-b">
                                                ...
                                                <th style={{width: '20%'}} className="p-3 text-left">
                                                    Nama TPS
                                                </th>
...
Tps.layout = page => <App children={page} />