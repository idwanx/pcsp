...
import DapilForm from './DapilForm';
import DangerButton from '@/Components/DangerButton';
import SecondaryButton from '@/Components/SecondaryButton';
import SideBar from './SideBar';

export default function Dapil(props) {
    const { data: dapil, meta } = props.dapil;
    ...
    const storeHandler = (e) => {
        e.preventDefault();
        post(route('datapemilu.storedapil'), {
            preserveScroll: true,
            onSuccess: () => {
                closeFormAdd();
            },
    ...
    const updateHandler = (e) => {
        e.preventDefault();
        put(route('datapemilu.updatedapil', data.id), {
            preserveScroll: true,
            onSuccess: () => {
                closeFormUpdate();
            },
    ...
    const destroyHandler = () => {
        router.delete(route('datapemilu.destroydapil', state.id), { 
            preserveScroll: true,
            onSuccess: () => {
                closeDestroy();
            },
    ...
    return (
        <>
            ...
            <Modal isOpen={isFormAdd} closeable={false} onClose={closeFormAdd} size={`sm`}>
                <DapilForm isClose={closeFormAdd} data={data} setData={setData} errors={errors} processing={processing} submit={storeHandler} submitLabel={`Tambah`} title={`Tambah Dapil`} />
            </Modal>
            <Modal isOpen={isFormUpdate} closeable={false} onClose={closeFormUpdate} size={`sm`}>
                <DapilForm isClose={closeFormUpdate} data={data} setData={setData} errors={errors} processing={processing} submit={updateHandler} submitLabel={`Update`} title={`Update Dapil`} />
            </Modal>
                ...
                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        <p className="text-sm text-gray-500">
                            Apakah anda yakin <span className='text-red-500'>{state.nama_dapil}</span> akan dihapus?
                        </p>
                        </div>
                    </div>
                    </div>
...
Dapil.layout = page => <App children={page} />