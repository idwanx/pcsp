...
export default function SideBar() {
  ...
  return (
    ...
    <div className="bg-white p-3 shadow-sm sm:rounded-lg lg:flex flex-shrink-0 flex-col gap-y-2 border border-gray-200">
        <ResponsiveSideBarLink href={route('laporansuara.dashboard', { partai: partai, tahun: tahun })} active={route().current('laporansuara.dashboard')}>
        ...
        {menupemilu?.map(item => {
            return (
              <ResponsiveSideBarLink key={item.id} href={route('laporansuara.pemilu', { partai: partai, tahun: tahun, pemilu: item.id })} active={pemilu ? pemilu.id == item.id : ''}>
